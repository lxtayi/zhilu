...
async function staticApi(path, options={}){const input=JSON.parse(options.body||'{}');let data;if(path.endsWith('health'))data={dataMode:'demo'};else if(path.endsWith('icebreakers'))data=buildIcebreakers(input);else{data=buildDemoExploreResult(input.question,buildQuestionAnalysis(input.question));data.warnings=[{message:'静态演示：人物、观点和日志示例均为合成内容。新分享仅保存在此浏览器，不会公开发布。'}];}return {ok:true,json:async()=>data};}