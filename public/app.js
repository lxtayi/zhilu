...
async function submitQuestion(question) {
  ...
  try {
    ...
    showGateWarnings(payload.warnings || []);
...
function showGateWarnings(warnings) {
  let notice = document.querySelector("#viewpointGateWarnings");
  if (!notice) {
    notice = document.createElement("p");
    notice.id = "viewpointGateWarnings";
    notice.setAttribute("role", "status");
    notice.setAttribute("aria-live", "polite");
    const title = document.querySelector("#viewpointGateTitle");
    title?.insertAdjacentElement("afterend", notice);
  }
  notice.textContent = warnings.filter((warning) => warning?.message).map((warning) => warning.message).join(" ");
